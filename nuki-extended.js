...
let setup = [];
let BRIDGES = {};
let nukiWebApi = new WebApiHandler(),
    listener = false;
...
function startAdapter(options) {
    ...
    adapter.on('ready', function () {
        ...
        if (version <= 6) {
            return library.terminate(
                `This Adapter is not compatible with your Node.js Version ${process.version} (must be >= Node.js v7).`,
                true,
            );
        }
        ...
        if (adapter.config.callbackPort < 10000 || adapter.config.callbackPort > 65535) {
            adapter.log.warn(
                `The callback port (${adapter.config.callbackPort}) is incorrect. Provide a port between 10.000 and 65.535! Using port 51989 now.`,
            );
            adapter.config.callbackPort = 51989;
        }
        adapter.config.additionalWebApiTimeout = parseInt(adapter.config.additionalWebApiTimeout, 10) || 3;
        adapter.config.additionalWebApiCall =
            adapter.config.additionalWebApiCall !== undefined ? adapter.config.additionalWebApiCall : true;
        ...
        adapter.getStates(`${adapterName}.${adapter.instance}.*`, (err, states) => {
            ...
            for (let state in states) {
                library.setDeviceState(
                    state.replace(`${adapterName}.${adapter.instance}.`, ''),
    ...
    adapter.on('stateChange', function (node, object) {
        ...
        if (state === '_delete' && object && object.ack !== true) {
            ...
            if (callbackIndex > -1) {
                BRIDGES[bridgeId].callbacks[callbackIndex]
                    .remove()
                    .then(() => {
                        ...
                        library._setValue(
                            `${BRIDGES[bridgeId].data.path}.callbacks.list`,
                            JSON.stringify(
                                BRIDGES[bridgeId].callbacks.map(cb => {
        ...
        // configuration
        if (
            (node.indexOf('.config.') > -1 ||
                node.indexOf('.advancedConfig.') > -1 ||
                node.indexOf('.openerAdvancedConfig.') > -1) &&
            nukiWebApi.active &&
            object &&
            object.ack !== true
        ) {
            ...
            if (node.indexOf('.config.') > -1) {
                nukiWebApi.setConfig(device.smartlockId, { ...device.config, [state]: action }, state);
            }

            // change advancedConfig
            else if (node.indexOf('.advancedConfig.') > -1) {
                nukiWebApi.setAdvancedConfig(device.smartlockId, { ...device.advancedConfig, [state]: action }, state);
            }
...
function initNukiAPIs() {
    ...
    listener = adapter.config.bridges.map(device => {
        ...
        if (!device.bridge_name || !device.bridge_ip || !device.bridge_token) {
            adapter.log.warn(
                `Name, IP or API token missing for bridge ${device.bridge_name}! Please go to settings and fill in IP and the API token first!`,
            );
        ...
        try {
            let bridge = {
                ...
                instance: new Bridge.Bridge(device.bridge_ip, device.bridge_port || 8080, device.bridge_token, {
                    forcePlainToken: adapter.config.hashedToken !== true,
                }),
            };
            ...
            adapter.log.error(
                `Error while initializing bridge ${device.bridge_name} on ${device.bridge_ip}:${device.bridge_port || 8080}: ${err.message}`,
            );
            return Promise.resolve(false);
        }
    });
    ...
    return Promise.all(listener)
        .then(values => {
            ...
            if (values.findIndex(el => el === 'attachListener') > -1) {
                ...
                _http.post('/nuki-api-bridge', (req, res) => {
                    if (req && req.body) {
                        ...
                        library.set(
                            library.getNode('bridgeApiLast'),
                            `${new Date().toISOString().substr(0, 19)}+00:00`,
                        );
            ...
            // no callback
                ...
                values.findIndex(el => el === 'attachListener') === -1 &&
                values.findIndex(el => el === 'doNotAttachListener') > -1
            ) {
            ...
            // no bridges
                ...
                adapter.log.info(
                    'No bridges are sufficiently defined! Name, IP or token missing or all bridges deactivated!',
                );
            }
        ...
        .then(cbs => {
            ...
            if (adapter.config.refreshBridgeApiType === 'callback') {
                ...
                if (BRIDGES[bridge.data.bridge_id].callbacks.findIndex(cb => cb.url === url) === -1) {
                    adapter.log.debug(
                        `Adding callback with URL ${url} to Nuki Bridge with name ${bridge.data.bridge_name}.`,
                    );

                    // set callback on bridge
                    bridge.instance
                        .addCallback(adapter.config.callbackIp || _ip.address(), adapter.config.callbackPort, false)
                        .then(res => {
                            if (!res || !res.url) {
                            ...
                            adapter.log.info(
                                `Callback (with URL ${res.url}) attached to Nuki Bridge with name ${bridge.data.bridge_name}.`,
                            );
                        ...
                        .catch(err => {
                            if (err && err.error && err.error.message === 'callback already added') {
                                adapter.log.debug(
                                    `Callback (with URL ${url}) already attached to Nuki Bridge with name ${bridge.data.bridge_name}.`,
                                );
                            } else if (BRIDGES[bridge.data.bridge_id].callbacks.length >= 3) {
                                adapter.log.warn(
                                    'Callback not attached because too many Callbacks attached to the Nuki Bridge already! Please delete a callback!',
                                );
                            } else {
...
function getBridgeApi(bridge) {
    ...
    adapter.log.silly(
        `Retrieving from Nuki Bridge API (Bridge ${bridge.data.bridge_ip}${adapter.config.hashedToken ? ' using hashed token' : ''})..`,
    );
    bridge.instance
        .list()
        .then(nukis => {
            adapter.log.debug(
                `getBridgeApi() [forcePlainToken: ${bridge.instance.forcePlainToken === true}]: ${JSON.stringify(nukis)}`,
            );
            nukis.forEach(nuki => {
                // remap states
        ...
        .catch(err => {
            adapter.log.warn(
                `Failed retrieving /list from Nuki Bridge with name ${bridge.data.bridge_name} (forcePlainToken: ${bridge.instance.forcePlainToken === true})!`,
            );
    ...
    bridge.instance
        ...
        .then(payload => {
			...
			*/
            ...
            library.set({
                node: bridge.data.path,
                description: `Bridge ${bridge.data.bridge_name ? `${bridge.data.bridge_name} ` : ''}(${bridge.data.bridge_ip})`,
                role: 'channel',
            });
        ...
        .catch(err => {
            adapter.log.warn(
                `Failed retrieving /info from Nuki Bridge with name ${bridge.data.bridge_name} (forcePlainToken: ${bridge.instance.forcePlainToken === true})!`,
            );
            adapter.log.debug(`getBridgeApi(): ${err.message}`);
        });
}
...
function setAction(device, action, api = 'bridge', retry = 0) {
    adapter.log.info(
        `Trigger action -${action.name}- on Nuki ${device.type} ${device.name} (via ${library.ucFirst(api)} API).`,
    );
    ...
    if (device.instance !== undefined && device.instance !== null && api === 'bridge') {
        ...
        device.instance
            .lockAction(action.id)
            .then(() => {
                adapter.log.info(
                    `Successfully triggered action -${action.name}- on Nuki ${device.type} ${device.name} (via Bridge API).`,
                );
                return Promise.resolve(true);
            })
            .catch(err => {
                adapter.log.warn(
                    `Error triggering action -${action.name}- on Nuki ${device.type} ${device.name} (via Bridge API). See debug log for details.`,
                );
    ...
    // try Web API
        ...
        nukiWebApi.api
            .setAction(device.smartlockId, action.id)
            .then(() => {
                adapter.log.info(
                    `Successfully triggered action -${action.name}- on Nuki ${device.type} ${device.name} (via Web API).`,
                );
                return Promise.resolve(true);
            })
            .catch(err => {
                adapter.log.warn(
                    `Error triggering action -${action.name}- on Nuki ${device.type} ${device.name} (via Web API). See debug log for details.`,
                );
                adapter.log.debug(err.message);
    ...
    adapter.getStates(`${path}.*`, (err, states) => {
        ...
        BRIDGES[bridgeId].callbacks.forEach(cb => {
            ...
            if (index === -1) {
                ...
                library.set(
                    { ...library.getNode('callbacks.callback.id'), node: `${node}.id` },
                    parseInt(cb.callbackId),
                );
        ...
        for (let key in urls) {
            library.del(`${path}.${key}`, true, () =>
                adapter.log.debug(`Deleted states for callback with URL ${urls[key]}.`),
            );
        }
    });
}

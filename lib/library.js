...
class Library
{
	...
	getNode(node)
	{
		return this._nodes[this.clean(node)] || this._nodes[this.clean(node.replace(RegExp(/\.\d+\./, 'g'), '.'))] || { 'description': '(no description given)', 'role': 'state', 'type': 'string', 'convert': null };
	}
	...
	{
		if (this._adapter && this._adapter.log && ((this.options.updatesInLog && !this.options.updatesExceptions) || (this.options.updatesInLog && this.options.updatesExceptions && Array.isArray(this.options.updatesExceptions) && this.options.updatesExceptions.indexOf(key.substr(key.lastIndexOf('.')+1)) == -1)))
			this._adapter.log.debug('Updated state ' + key + ' to value ' + value + ' (from ' + this._STATES[key] + ').');
	...
	set(node, value)
	{
		...
		// catch error
		if (!node.node || (!node.name && !node.description))
		{
	 ...
	 * @param	{boolean}	force		Force to set value
	 * @return	void
	 *
	 */
	async _setValue(state, value, force)
	{
		if (force || (value !== undefined && (this._STATES[state] != value || this._STATES[state] === undefined)))
		{

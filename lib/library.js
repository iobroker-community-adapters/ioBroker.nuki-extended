...
class Library
{
    ...
    constructor(adapter, options)
	{
		...
		this.options.updatesInLog = options.updatesInLog || false;
		this.options.updatesExceptions = options.updatesExceptions || ['timestamp', 'datetime', 'UTC', 'localtime', 'last_use_date'];
	...
	terminate(message, reason)
	{
		...
		if (this._adapter && this._adapter.terminate)
		...
		else
		{
			...
			}, 100); // delay necessary to actually show the error message
		}
	 ...
	 * @return	{boolean}				Indication if value has been updated
	 *
	 */
	_setValue(state, value)
	{
		if (value !== undefined && (this._STATES[state] != value || this._STATES[state] === undefined))
		{
			...
			this._adapter.setState(state, { val: value, ts: Date.now(), ack: true });

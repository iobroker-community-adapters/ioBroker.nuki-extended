...
class Library
{
	 ...
	 *
	 * @param	{string}	property				Property of state as selection
	 * @param	{string}	value					Value of the property
	 * @param	{number}	ts						Objects older than this timespan will be deleted (in ms)
	 ...
	 *
	 */
	runGarbageCollector(property, value, ts = null, del = true, offset = 60)
	{
		if (property === undefined || value === undefined)
			return false;
		
		ts = ts ? ts-offset*1000 : ts;
		for (let key in this._STATES)
		{
			if (this._STATES[key][property] && this._STATES[key][property] == value && (ts === null || (ts && this._STATES[key]['ts'] < ts)))
			{
				this._adapter.log.debug('Garbage Collector: ' + (del ? 'Deleted ' : 'Emptied ') + key + '!');
				
				if (del)
				{
					this._adapter.delObject(key);
					this._STATES[key] = undefined;
				}
				else
					this._setValue(this._STATES[key], '', { 'force': true });
			}
		}
	}
	 ...
	 *
	 */
	setDeviceState(state, value, properties = {})
	{
	...
	{
		this._STATES[state] = Object.assign(this._STATES[state] || {}, properties);
		return true;
	}
	...
	set(node, value, options = {})
	{
		...
		if (this._STATES[node.node] === undefined)
			return this._createNode(node, () => that._setValue(node.node, value, options));
		
		// set value
		else
			return this._setValue(node.node, value, options);
	}
	...
	_createNode(node, callback)
	{
		...
		this._adapter.setObjectNotExists(
			node.node,
			{
				'common': Object.assign({ 'name': node.name, 'role': 'state', 'type': 'string', 'write': false }, node.common || {}, common),
				'type': 'state',
				'native': node.native || {}
			},
			function(err, obj)
			{
				if (obj !== undefined && that._adapter && that._adapter.log)
	 ...
	 * @param	{object}	[options.properties]	Additional properties of the state
	 * @return	void
	 *
	 */
	async _setValue(state, value, options = {})
	{
		if (state !== undefined)
		{
			...
			{
				this.setDeviceState(state, value, options.properties);
				await this._adapter.setStateAsync(state, { val: value, ts: Date.now(), ack: true });
			}
			else
				this.setDeviceProperties(state, options.properties);
		}
	}

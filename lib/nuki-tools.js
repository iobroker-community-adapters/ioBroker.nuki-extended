...
class NukiTools {
    ...
    static updateLock(payload, library, adapter) {
        ...
        if (this.DEVICES[payload.nukiHexId] === undefined || !this.DEVICES[payload.nukiHexId].path) {
            ...
            // Nuki Smartlock 4.x / Pro (deviceType 5)
            if (payload.deviceType === 5) {
                library.set(library.getNode('smartlocks'));
                type = 'Smartlock';
                actions = _LOCK.ACTIONS;
            }
            ...
            // index device
            path = `${(type || 'Smartlock').toLowerCase()}s.${library.clean(payload.name, true, '_')}`;
            this.DEVICES[payload.nukiHexId] = {
                id: payload.nukiId,
                hex: payload.nukiHexId,
                smartlockId: parseInt(payload.deviceType + payload.nukiHexId, 16),
                name: payload.name,
                type: type,
                path: path,
                bridge: null,
            ...
            if (actions !== null) {
                ...
                library.set(
                    { ...library.getNode('action'), node: actionPath, common: { write: true, states: actions } },
                    0,
                );
                ...
                for (let key in actions) {
                    if (key > 0) {
                        ...
                        library.set(
                            {
                                description: `Trigger ${action} action`,
                                type: 'boolean',
                                role: 'button',
                                node: `${actionPath}.${action.replace(/ /g, '_')}`,
                                common: { write: true },
                            },
                            false,
                        );
        ...
        // update state
        if (payload.deviceType === 2 && payload.state && payload.state.state === 1 && payload.mode === 3) {
            // change ONLINE & CONTINOUS to RING_TO_OPEN
        ...
        if (
            payload.state &&
            payload.state.state &&
            payload.state.state !== library.getDeviceState(`${path}.state.lockState`)
        ) {

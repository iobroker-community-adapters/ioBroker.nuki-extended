...
class WebApiHandler {
    ...
    initialize(adapter, library) {
        ...
        if (!adapter.config.refreshWebApi) {
            ...
            adapter.log.warn(
                'Due to performance reasons, the refresh rate can not be set to less than 5 seconds. Using 5 seconds now for Nuki Web API.',
            );
    ...
    getWebApi() {
        ...
        this.library.set(this.library.getNode('webApiSync'), true, { force: true });
        this.library.set(this.library.getNode('webApiLast'), `${new Date().toISOString().substring(0, 19)}+00:00`);
    ...
    processSmartLocksResponse(smartlocks) {
        ...
        smartlocks.forEach(smartlock => {
            ...
            // get logs
            this.api
                .getSmartlockLogs(smartlock.smartlockId, { limit: 1000 })
                .then(log => {
                    this.library.set(
                        {
            ...
            this.api
                .getSmartlockAuth(smartlock.smartlockId)
                .then(users => {
                    this.library.set({
                    ...
                    users.forEach(user => {
                        ...
                        let nodePath = `${NukiTools.DEVICES[smartlock.nukiHexId].path}.users.${this.library.clean(
                            user.name,
                            true,
                            '_',
                        )}`;
    ...
    setConfig(smartLockID, configuration, state) {
        this.api
            .setConfig(smartLockID, configuration)
            .then(() => {
    ...
    setAdvancedConfig(smartLockID, configuration, state) {
        this.api
            .setAdvancedConfig(smartLockID, configuration)
            .then(() => {
                setTimeout(this.getWebApi.bind(this), 3 * 1000);

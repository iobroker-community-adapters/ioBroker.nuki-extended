	...
	adapter.on('stateChange', function(node, object)
	{
		...
		if (state === 'action' && Number.isInteger(action) && action > 0 && object.ack !== true)
		{
			...
			adapter.getObject(node, function(err, node)
			{
				...
				if (err !== null || !nukiId)
				{
					adapter.log.warn('Error triggering action -' + _LOCK.ACTIONS[action] + '- on the Nuki: ' + (err ? err.message : 'No Nuki ID given!'));
					return;
				}
				
				// log
				adapter.log.info('Triggered action -' + _LOCK.ACTIONS[action] + '- on Nuki ' + doors[nukiId].name + '.');
				...
				if (bridge !== null)
				{
					...
					bridge.get(nukiId).then(function(device)
					{
						device.lockAction(action)
							.then(function()
							{
								adapter.log.info('Successfully triggered action -' + _LOCK.ACTIONS[action] + '- on Nuki ' + doors[nukiId].name + '.');
								library._setValue(node, 0);
							})
							.catch(function(e)
							{
								adapter.log.warn('Error triggering action -' + _LOCK.ACTIONS[action] + '- on Nuki ' + doors[nukiId].name + '. See debug log for details.');
...
function updateLogs(nukiId)
{
	...
	{
		library.set({node: doors[nukiId].device + '.logs', description: 'Logs / History of Nuki'}, JSON.stringify(log.slice(0, 250)));
